...
# -*- coding: utf-8 -*-
"""
Created on Tue Feb 14 09:37:54 2023
...
positiontitle = fm.FontProperties(fname=fontPathNBold, size=26)
playertitle = fm.FontProperties(fname=fontPathBold, size=38)
teamtitle = fm.FontProperties(fname=fontPathBold, size=32)
seasontitle = fm.FontProperties(fname=fontPathBold, size=28)
subtitles = fm.FontProperties(fname=fontPathNBold, size=26)
...
def load_ncaa_data():
    #url = 'https://drive.google.com/file/d/1KD5nxMlZZImiArxLXg4N43uiUkJ4taQP/view?usp=sharing'
    #path = 'https://drive.google.com/uc?export=download&id='+url.split('/')[-2]
    path = './Wyscout_NCAA_All_Data.csv'
    df = pd.read_csv(path)
    df.Team = df['Team']
    #df = df[(df.Team.isin(player)) & (df.Season.isin(season))]
    return df
...
def ncaa_positional_zscore_df(state):
    ...
    with col2:
        conference = st.multiselect('Select Conference Level', natsorted(df1.conference_type.unique()), default="Power 5")
    league_df = df1[(df1['conference_type'].isin(conference)) & (df1['Season'] == (season))]
    with col3:
        position = st.selectbox('Select Position', ['CF', 'W', 'AM-CM', 'DM', 'FB', 'CB'])
